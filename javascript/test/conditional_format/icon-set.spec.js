...
test('save to file with conditional format ("ConditionalFormatIconSet")', async (t) => {
  ...
  sheet.writeString(
    2,
    0,
    'Three Traffic lights - Red is highest',
    centerFormat,
  );
  ...
  await workbook.saveToFile(`${path}/save-to-file-with-icon-set.xlsx`);
  assert(fs.existsSync(`${path}/save-to-file-with-icon-set.xlsx`));
});

test('Should trow error when number of icons is not correct for the type of icon set', async (t) => {
  const defaultRulesForThree = [
    new ConditionalFormatCustomIcon({
      iconRule: {
        type: 'percent',
  ...
  assert.throws(() => {
    const rule = new ConditionalFormatIconSet({
      iconType: 'threeTrafficLights',
      icons: defaultRulesForThree,
    });
    rule.setIcons('fiveArrows', defaultRulesForThree);
  });
});
